...
SpawnerCatalog = (function()
	...
	local function ensureImageIndex()
		...
		local candidates = {
			"catalog_full.json",
			"torti_spawn_catalog_repo/catalog_full.json",
			"torti_github_repo/catalog_full.json",
		}
	...
	local function getImageMeta(entry)
		ensureImageIndex()
		local displayName = BuildSpawnerButtonLabel(entry, false)
		local names = { displayName, entry.name }
	...
	local function queueCardImage(record)
		...
		task.spawn(function()
			...
			local fileName = string.gsub(NormalizeItemName(BuildSpawnerButtonLabel(entry, false)), "[^%w]+", "_")
	...
	function module.CreateCard(entry, tradable, defaultOrder)
		...
		fallbackLabel.Text = string.upper(string.sub(BuildSpawnerButtonLabel(entry, false), 1, 1))
		...
		nameLabel.Text = BuildSpawnerButtonLabel(entry, false)
	...
	function module.Refresh(records, query, sortMode)
		...
		for index, info in ipairs(ordered) do
			local haystack = normalizeWeaponName(("%s %s %s %s"):format(
				tostring(BuildSpawnerButtonLabel(info.entry, false)),
